...
fn test_xnpv_samples(#[case] input: &str, #[case] expected: f64) {
    ...
    // Call xnpv directly
    let result = xnpv(rate, &dates, &amounts, None).unwrap();
...
fn test_xirr_samples(#[case] input: &str, #[case] expected: f64) {
    ...
    // Call xirr directly
    let result = xirr(&dates, &amounts, None, None).unwrap_or(f64::NAN);
    ...
    if result.is_nan() {
        ...
        assert_almost_eq!(xnpv_result, 0.0, 1e-3);
    }
}
...
fn test_xirr_silent() {
    ...
    // Check that calling xirr with invalid inputs returns an error
    let result = xirr(&dates, &amounts, None, None);
    assert!(result.is_err());

    // In pure Rust, we can't test the "silent" parameter directly since it's
...
#[rstest]
fn test_xfv() {
    // Test case from: http://westclintech.com/SQL-Server-Financial-Functions/SQL-Server-XFV-function
    let start_date = date!(2011 - 02 - 01).into();
    let investment_date = date!(2011 - 03 - 01).into();
    let end_date = date!(2012 - 02 - 01).into();
    let rate_from_start_to_investment = 0.00142;
    let rate_from_investment_to_end = 0.00246;
    let amount = 100000.0;
    ...
    let result = xfv(
        ...
        &investment_date,
        &end_date,
        rate_from_start_to_investment,
        rate_from_investment_to_end,
        amount,
        None,
    );
...
#[rstest]
fn test_xnfv() {
    // Load the data directly with Rust
    let payments = load_payments_from_csv("tests/samples/xnfv.csv").unwrap();
    let (dates, amounts) = split_payments(&payments);

    let rate = 0.0250;
    let result = xnfv(rate, &dates, &amounts, None).unwrap();
...
#[rstest]
fn test_xnfv_silent() {
    // Create test data with invalid setup (different length arrays)
    let dates = vec![date!(2021 - 01 - 01), date!(2022 - 01 - 01)]
        .into_iter()
        .map(|x| x.into())
        .collect::<Vec<_>>();
    let amounts = vec![1000.0, 100.0, 50.0]; // One more amount than dates

    // Test that it returns an error
    let result = xnfv(0.0250, &dates, &amounts, None);
    assert!(result.is_err());
...
fn test_sum_xfv_eq_xnfv() {
    ...
    let sum_xfv_result: f64 = dates
        .iter()
        .zip(amounts.iter())
        .map(|(date, amount)| xfv(&min_date, date, &max_date, rate, rate, *amount, None))
        .sum();
...
fn parse_date(date_str: &str) -> Result<Date, Box<dyn std::error::Error>> {
    // Try different formats (MM/DD/YYYY)
    if let Ok(date) =
        Date::parse(date_str, &time::format_description::parse("[month]/[day]/[year]").unwrap())
    {
        return Ok(date);
    }
    ...
    if let Ok(date) =
        Date::parse(date_str, &time::format_description::parse("[year]-[month]-[day]").unwrap())
    {
        return Ok(date);
    }

    Err("Could not parse date".into())
}
...
fn test_xirr_day_count(#[case] day_count: &str, #[case] expected: f64) {
    ...
    let date_strs = ["01/12/2007", "02/14/2008", "03/03/2008", "06/14/2008", "12/01/2008"];
    let amounts = [-10000.0, 2500.0, 2000.0, 3000.0, 4000.0];

    // Parse date strings to Date objects
    let dates: Vec<DateLike> = date_strs.iter().map(|s| parse_date(s).unwrap().into()).collect();

    let dc = DayCount::from_str(day_count).unwrap();

    // Pass day_count as an option
    let result = xirr(&dates, &amounts, None, Some(dc)).unwrap();
